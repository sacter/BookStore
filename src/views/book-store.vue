<style>
    .book-store .ivu-btn{
        padding: 4px 10px;
    }
    .book-store .ivu-page{
        text-align: right;
        margin-top: 10px;
        margin-bottom: 30px;
    }
    .comp-title{
        width: 100%;
        padding:10px 0;  
    }
    .comp-title h2{
        font-size: 18px;
        text-align: left;
    }
    .comp-title  .ivu-btn{
        height: 30px;
        padding: 2px 10px;
    }
    .ivu-btn-primary,.ivu-input-search,.ivu-input-search:hover{
        background-color: #169BD5 !important;
        border-color: #169BD5 !important;
    }
    .book-store-search{
        margin: 10px 0 10px;
    }
    .book-store-search>div{
        margin-right: 20px;
    }
    .book-store-btns{
        position: absolute;
        right: 0;
    }
    .book-store-btns Button{
        margin-left: 12px;
    }
    .book-store-filter{
        margin-bottom: 20px;
    }
    .book-store-filter>div{
        margin-bottom: 10px;
    }
    .ivu-modal{
        top:20%;
    }
    .ivu-btn-text:focus{
        box-shadow: none;
    }
    .price-cont{
        margin-bottom: 10px;
    }
    .classification .condition-title,.classification .all{
            visibility: hidden;
    }
</style>
<template>
    <div class="book-store">
        <div class="comp-title flex justify">
            <h2>书籍列表</h2>
        </div>
        <div class="book-store-filter">
            <FilterItem class="filter-item"  :isMulti=false :filterData="comefrom" :filterName="'来源'" @filterCondition-change="bookSel($event,'comefrom')"></FilterItem>
            <FilterItem class="filter-item"  :isMulti=false :filterData="channel" :filterName="'分类'" @filterCondition-change="selectChannel"></FilterItem>
            <FilterItem class="filter-item classification"  :isMulti=false :filterData="classification" :filterName="'分类'" @filterCondition-change="bookSel($event,'classification')"></FilterItem>
            <FilterItem class="filter-item"  :isMulti=false :filterData="wordsNum" :filterName="'字数'" @filterCondition-change="bookSel($event,'wordsNum')"></FilterItem>
            <FilterItem class="filter-item"  :isMulti=false :filterData="chapters" :filterName="'章节'" @filterCondition-change="bookSel($event,'chapters')"></FilterItem>
            <FilterItem class="filter-item"  :isMulti=false :filterData="status" :filterName="'状态'" @filterCondition-change="bookSel($event,'status')"></FilterItem>
            <FilterItem class="filter-item"  :isMulti=false :filterData="onShelfs" :filterName="'上架'" @filterCondition-change="bookSel($event,'onShelfs')"></FilterItem>
        </div>
        <div class="book-store-search flex">
            <Input search enter-button v-model="bookListObj.search" @on-search="searchFun" placeholder="请输入关键字搜索" style="width: auto"/>
            <div class="book-store-btns flex">
                <Button type="primary" :size="large"  @click="upload">上传作品</Button>
                <Button type="primary" :size="large" @click="shelf(1)">上架</Button>
                <Button type="primary" :size="large" @click="shelf(0)">下架</Button>             
                <Button type="primary" :size="large" @click="openModal1">修改定价</Button>
                <Button type="primary" :size="large" @click="openModal2">设置收费章节</Button>
            </div>
        </div>
        <div class="book-store-tab">
            <Table border ref="selection" :columns="columns" :data="data" @on-select="tabSel" @on-select-cancel="tabDel" @on-select-all="tabAll" @on-selection-change="tabChange"></Table>
            <Page :total="totalElements" show-total transfer placement="bottom" @on-change="changePage" @on-page-size-change="changeSize" show-elevator show-sizer/>
        </div>
        <!-- <Modal
            v-model="modal1"
            title="修改定价"
            width="50%"
            @on-ok="priceOK">
            <div class="price-cont" v-for="(book,index) in selectArr" :key="book.id">
                <h3>{{index + 1}}、{{book.name}}</h3>
                <div>
                    <p>原价：{{book.fee}} 书币、千字</p>
                    <div>
                        修改价格：<Input v-model="book.newfee" placeholder="请输入价格" style="width: 80px"/> 书币/千字
                    </div>
                </div>
            </div>
        </Modal> -->
        <Modal v-model="modal1" title="修改定价" width = 50% :mask-closable="false" class-name="vertical-center-modal">
            <Form ref="formInline">
                <FormItem prop="newfee">
                    <div class="price-cont" v-for="(book,index) in selectArr" :key="book.id">
                        <h3>{{index + 1}}、{{book.name}}</h3>
                        <div>
                            <p>原价：{{book.fee}} 书币、千字</p>
                            <div>
                                修改价格：<Input v-model="booknewfee" placeholder="请输入价格" style="width: 80px"/> 书币/千字
                            </div>
                        </div>
                    </div>
                </FormItem>
            </Form>
            <div slot="footer">
                <Button @click="cancel()">取&emsp;消</Button>
                <Button type="primary" @click="priceOK()">确&emsp;定</Button>
            </div>
        </Modal>
        <Modal
            v-model="modal2"
            title="设置收费章节"
            width="50%"
            :mask-closable="false" class-name="vertical-center-modal">
            <div class="price-cont" v-for="(book,index) in selectArr" :key="book.id">
                <h3>{{index + 1}}、{{book.name}}</h3>
                <div>
                    <div>
                        收费章节：<Input v-model="bookfeeChapter" placeholder="请输入章节" style="width: 80px"/> 章以后
                    </div>
                </div>
            </div>
            <div slot="footer">
                <Button @click="cancel()">取&emsp;消</Button>
                <Button type="primary" @click="moneyOk()">确&emsp;定</Button>
            </div>
        </Modal>
    </div>
</template>
<script>
    import FilterItem from '@/components/filter_item.vue'
    import {wordsNum,chapters,status,onShelfs,channel} from '../const-data/list-key.js'

    export default {
        components: {FilterItem},
        created () {
            let promise1 = new Promise((resolve, reject) => {
                this.$axios.get(this.URL + 'adminUserAuth/bookstore/getBookSource').then(res =>{
                    let result = res.data
                    if (result.code == 200) {
                        this.comefrom = this.setData(result.data)
                    }
                    if (result.errorCode == 90002) {
                        this.$router.push('/')
                    }
                })
            })
            let promise2 = new Promise((resolve, reject) => {
                this.$axios.get(this.URL + 'adminUserAuth/bookstore/getCategory').then(res =>{
                    let result = res.data
                    if (result.code == 200) {
                        this.classification = result.data.man.concat(result.data.woman)   
                    }   
                    if (result.errorCode == 90002) {
                        this.$router.push('/')
                    }
                })
            })
            let promise3 = new Promise((resolve, reject) => {
                this.pageSize = 10
                this.page = 0
                setTimeout(() => {
                    this.getBookList()
                }, 100);
                
            })
            Promise.all([promise1,promise2,promise3])
        },
        data () {
            return {
                modal1: false, // 修改定价modal
                modal2: false, // 设置收费章节modal
                booknewfee:'',
                bookfeeChapter:'',
                large: 'large',
                totalElements:10, // 总记条数
                page:0, // 当前第几页
                pageSize:10, // 每页显示几条
                comefromKey:'', // 来源条件
                ficatKey:'', // 分类条件
                statusKey:'', //状态条件
                shelfKey:'', //上架条件
                selectArr:[], // 选中图书数组
                bookListObj: { // 书籍列表对象
                    search:'',
                    channel:'',
                    categoryId:'',
                    source:'',
                    end:'',
                    charge:'',
                    onShelf:'',
                    chapterNum:'',
                    wordNum:'',
                    pageSize:'',
                    pageNum:''
                }, 
                comefrom: [], //来源
                classification: [], //分类
                wordsNum: wordsNum, //字数
                chapters: chapters, //章节
                status: status, //状态
                onShelfs: onShelfs, //上架
                channel:channel, // 频道
                columns: [
                    {
                        type: 'selection',
                        width: 60,
                        align: 'center'
                    },
                    {
                        title: '书号',
                        key: 'bookNum',
                        align: 'center'
                    },
                    {
                        title: '书名',
                        key: 'name',
                        align: 'center'
                    },
                    {
                        title: '频道',
                        key: 'channel',
                        align: 'center'
                    },
                    {
                        title: '分类',
                        key: 'category',
                        align: 'center'
                    },
                    {
                        title: '章节数',
                        key: 'chapterNum',
                        align: 'center'
                    },
                    {
                        title: '字数',
                        key: 'wordCount',
                        align: 'center'
                    },
                    {
                        title: '状态',
                        key: 'over',
                        align: 'center'
                    },
                    {
                        title: '来源',
                        key: 'source',
                        align: 'center'
                    },
                    {
                        title: '上架状态',
                        key: 'status',
                        align: 'center'
                    },
                    {
                        title: '操作',
                        key: 'operation',
                        align: 'center',
                        render: (h, params) => {
                            return h('div', [
                                h('a', {
                                    style: {
                                        color: '#169BD5',
                                        marginRight: '30px'
                                    },
                                    on: {
                                        click: () => {
                                            sessionStorage.setItem('BookDetail-bookId',JSON.stringify(params.row.bookId))
                                            this.$router.push('/BookDetail')
                                        }
                                    }
                                }, '查看'),
                                h('a', {
                                    style: {
                                        color: '#169BD5'
                                    },
                                    on: {
                                        click: () => {
                                            sessionStorage.setItem('edit-bookId',params.row.bookId)
                                            this.$router.push('/Uploading')
                                        }
                                    }
                                }, '编辑')
                            ])
                        }
                    }
                ],
                data: [],
                
            }
        },
        methods: {
            upload () {
                sessionStorage.setItem('edit-bookId','')
                this.$router.push('/Uploading')
            },
            changePage (p) {
                this.page = p > 0 ? p - 1 : 0
                this.getBookList()
            },
            changeSize (s) {
                this.pageSize = s
                this.getBookList()           
            },
            selectChannel (s) { // 选择男频、女频
                let sex = ''
                this.bookListObj.channel = ''
                this.bookListObj.categoryId = ''
                if (s.length) {
                    sex = s[0].id
                    this.bookListObj.channel = s[0].id
                }
                this.$axios.get(this.URL + 'adminUserAuth/bookstore/getCategory',{params:{sex:sex}}).then(res =>{
                    let result = res.data
                    if (result.code == 200) {
                        result.data.man = result.data.man || []
                        result.data.woman = result.data.woman || []
                        this.classification = result.data.man.concat(result.data.woman)   
                    }   
                })
                this.pageSize = 10
                this.page = 0
                this.getBookList()
            },
            bookSel(list, type) {
                if (type === 'comefrom') {
                    this.bookListObj.source = list[0]?list[0].id:''
                } else if (type === 'classification') {
                    this.bookListObj.categoryId = list[0]?list[0].id:''
                } else if (type === 'wordsNum') {
                    this.bookListObj.wordNum = list[0]?list[0].id:''
                } else if (type === 'chapters') {
                    this.bookListObj.chapterNum = list[0]?list[0].id:''
                } else if (type === 'status') {
                    this.bookListObj.end = list[0]?list[0].id:''
                } else if (type === 'onShelfs') {
                    this.bookListObj.onShelf = list[0]?list[0].id:''
                }
                this.pageSize = 10
                this.page = 0
                this.getBookList()
            },
            searchFun () {
                this.pageSize = 10
                this.page = 0
                this.getBookList()
            },
            tabSel (selection,row) {
                this.selectArr.push(row)
            },
            tabDel (selection,row) { // 删除
                this.selectArr.forEach((item,index) => {
                    if (item.bookId === row.bookId) {
                        this.selectArr.splice(index,1)
                    }
                })
            },
            tabChange  (selection) { // 全选删除
                let dataIds = this.getDataIds()
                if (!selection.length) {
                    dataIds.forEach(item => {
                        this.selectArr.forEach((ele,index) => {
                            if (ele.bookId === item) {
                                this.selectArr.splice(index,1)
                            }
                        })
                    })
                }
            },
            tabAll (selection) { // 全选
                let dataIds = this.getDataIds()
                dataIds.forEach(item => {
                    this.selectArr.forEach((ele,index) => {
                        if (ele.bookId === item) {
                            this.selectArr.splice(index,1)
                        }
                    })
                })
                this.selectArr = this.selectArr.concat(this.data)
            },
            getDataIds () {
                let arr = []
                this.data.forEach(item => {
                    arr.push(item.bookId)
                })
                return arr
            },
            openModal1 () {
                if (!this.selectArr.length) {
                    this.$Message.warning('请选择书籍!');
                    return;
                }
                this.modal1 = true
            },
            openModal2 () {
                if (!this.selectArr.length) {
                    this.$Message.warning('请选择书籍!');
                    return;
                }
                this.modal2 = true
            },
            priceOK () { // 修改价格
                let bookIdArr = []
                let valid = true
                this.selectArr.forEach(item => {
                    if (!item.newfee || !Number(item.newfee)) {
                        valid = false
                        return
                    }
                    bookIdArr.push(item.bookId)
                })
                if (!valid) {
                    this.$Message.warning('价格格式不正确!')
                    return
                }
                let param = new FormData(); //创建form对象
                param.append('bookIds',bookIdArr.join(','))
                param.append('fee',this.booknewfee)
                let config = {
                    headers:{'Content-Type':'multipart/form-data'}
                };
                this.$axios.post(this.URL + 'adminUserAuth/bookstore/updatePrice',param,config).then(res =>{
                    let result = res.data
                    if (result.code == 200) {
                        this.$Message.success('修改定价成功!')
                        this.pageSize = 10
                        this.page = 0
                        this.getBookList()
                    }
                    if (result.errorCode == 90002) {
                        this.$router.push('/')
                    }
                }).catch(err => {
                    console.log(err);
                })
                this.selectArr = []
                this.modal1 = false
            },
            cancel () {
                this.modal1 = false;
                this.modal2 = false;
            },
            moneyOk () { // 收费章节
                let bookIdArr = []
                this.selectArr.forEach(item => {
                    bookIdArr.push(item.bookId)
                })
                if (this.bookfeeChapter <= 0 ) {
                    this.$Message.warning('收费章节不能小于0')
                    return
                } else if (!Number(this.bookfeeChapter)) {
                    this.$Message.warning('收费章节格式不正确')
                    return
                }
                let param = new FormData(); //创建form对象
                param.append('bookIds',bookIdArr.join(','))
                param.append('ordinal',this.bookfeeChapter)
                let config = {
                    headers:{'Content-Type':'multipart/form-data'}
                };
                this.$axios.post(this.URL + 'adminUserAuth/bookstore/updateFeeChapter',param,config).then(res =>{
                    let result = res.data
                    if (result.code == 200) {
                        this.$Message.success('设置收费章节成功!')
                        this.pageSize = 10
                        this.page = 0
                        this.getBookList()
                    }
                    if (result.errorCode == 90002) {
                        this.$router.push('/')
                    }
                }).catch(err => {
                    console.log(err);
                })
                this.selectArr = []
                this.modal2 = false
            },
            shelf (type) { //上下架
                if (!this.selectArr.length) {
                    this.$Message.warning('请选择书籍!');
                    return;
                }
                let bookIdArr = []
                this.selectArr.forEach(item => {
                    bookIdArr.push(item.bookId)
                })
                let param = new FormData(); //创建form对象
                param.append('bookIds',bookIdArr.join(','))
                param.append('status',type)
                let config = {
                    headers:{'Content-Type':'multipart/form-data'}
                };
                this.$axios.post(this.URL + 'adminUserAuth/bookstore/onShelf',param,config).then(res =>{
                    let result = res.data
                    if (result.code == 200) {
                        let msg = type?'上架':'下架'
                        this.$Message.success(`${msg}成功!`)
                        this.pageSize = 10
                        this.page = 0
                        this.getBookList()
                    }
                    if (result.errorCode == 90002) {
                        this.$router.push('/')
                    }
                }).catch(err => {
                    console.log(err);
                })
            },
            getBookList () {
                this.bookListObj.pageSize = this.pageSize
                this.bookListObj.pageNum = this.page
                this.$axios.get(this.URL + 'adminUserAuth/bookstore/bookList',{params:this.bookListObj}).then(res =>{
                    let result = res.data
                    if (result.code == 200) {
                        this.data = result.data.content
                        this.totalElements = result.data.totalElements
                        this.data.forEach((item,index) => {
                            item.newfee = item.fee
                            item.newfeeChapter = item.feeChapter
                            item.bookNum = this.page * this.pageSize + index + 1
                            item.channel = item.channel?'男频':'女频'
                            item.over = item.over?'完结':'连载中'
                            item.status = item.status?'已上架':'未上架'
                            this.selectArr.forEach(sel => {
                                if (item.bookId === sel.bookId) {
                                    item._checked = true
                                }
                            });
                            this.comefrom.forEach(com => {
                                if (item.source == com.id) {
                                    item.source = com.name
                                }
                            })
                            this.classification.forEach(cla => {
                                if (item.category == cla.id) {
                                    item.category = cla.name
                                }
                            })
                        })
                    } 
                    if (result.errorCode == 90002) {
                        this.$router.push('/')
                    }
                }).catch(err => {
                    console.log(err);
                })
            },
            setData (arr) {
                if (arr && arr.length) {
                    arr.forEach(item => {
                        if (item.id || item.key) {
                            item.id = item.id || item.key
                        }
                        if (item.value || item.name) {
                            item.name = item.value || item.name
                        }
                    })
                    return arr
                }
            }
        }
    }
</script>
