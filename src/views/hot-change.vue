<template>
    <div class="book-store">
        ...
        <div class="book-store-search flex">
            ...
            <div class="book-store-btns flex" v-if="hotsubtitle === '添加'">          
        ...
        <Modal
            ...
            <div class="chapter-cont">
                <p style="width:100%">是否{{hotsubtitle}}书籍？</p>
            </div>
        </Modal>
    </div>
</template>
<script>
    ...
    export default {
        ...
        created () {
            this.hotsubtitle = sessionStorage.getItem('hotsubtitle')
            this.oldBookId = sessionStorage.getItem('hotbookId')
            this.columns = this.hotsubtitle === '添加'?this.columns1:this.columns2
        ...
        methods: {
            ...
            changeOk () { // 添加或更换书籍
                ...
                if (this.hotsubtitle === '添加') {
                    param.append('bookIds',bookIds.toString())
                    this.$axios.post(this.URL + 'adminUserAuth/bookstore/addHotRecomBooks',param,config).then(res =>{
                        let result = res.data
                        if (result.code == 200) {
                            this.$Message.success('书籍添加成功!')
                            this.$router.push('/PopularRecommend')
                        }
                        if (result.errorCode == 90002) {
                        this.$router.push('/')
                    }
                    ...
                    this.$axios.post(this.URL + 'adminUserAuth/bookstore/updateHotRecomBook',param,config).then(res =>{
                        let result = res.data
                        if (result.code == 200) {
